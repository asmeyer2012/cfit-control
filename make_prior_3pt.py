...
def make_prior_3pt(models,prior_dict=None,nst=-1,ost=-1):
 prior = gv.BufferDict()
  ...
  if prior_dict is None: ## -- take default from defines
   if (nst > -1 and ost > -1):
    dprior = utf.get_prior_dict(df.define_prior_3pt,
     ...
     df.define_prior_3pt['vkey'])
   else:
    dprior = utf.get_prior_dict(df.define_prior_3pt,
     df.define_prior_3pt['nkey'],df.define_prior_3pt['okey'],df.num_nst_3pt,df.num_ost_3pt,
     df.define_prior_3pt['vkey'])
   ...
   if (nst > -1 and ost > -1):
    dprior = utf.get_prior_dict(prior_dict,
     prior_dict['nkey'],prior_dict['okey'],nst,ost,prior_dict['vkey'])
   else:
    dprior = utf.get_prior_dict(prior_dict,
     prior_dict['nkey'],prior_dict['okey'],df.num_nst_3pt,df.num_ost_3pt,prior_dict['vkey'])
   for pkey in dprior[skey]:
    prior[pkey]=dprior[skey][pkey]
